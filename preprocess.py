...
def wavs_to_specs(wavs_mono, wavs_src1, wavs_src2, n_fft = 1024, hop_length = None):
    ...
    for wav_mono, wav_src1, wav_src2 in zip(wavs_mono, wavs_src1, wavs_src2):
        ...
        stft_src2 = librosa.stft(wav_src2, n_fft = n_fft, hop_length = hop_length)  #返回一个[513,3537]
...
def sample_data_batch(stfts_mono, stfts_src1, stfts_src2, batch_size = 64, sample_frames = 8):
    ...
    stft_mono_batch = list()   #513,10
    stft_src1_batch = list()
    stft_src2_batch = list()

    collection_size = len(stfts_mono)  #88
    collection_idx = np.random.choice(collection_size, batch_size, replace = True)  #随机选择64维的在88内的index [88 47 ....]
    for idx in collection_idx:
        stft_mono = stfts_mono[idx]   #(513,3501)
        stft_src1 = stfts_src1[idx]
        stft_src2 = stfts_src2[idx]
        num_frames = stft_mono.shape[1]  #3501
        assert  num_frames >= sample_frames
        start = np.random.randint(num_frames - sample_frames + 1)  #2105
        end = start + sample_frames  #2115
    ...
    # Shape: [batch_size, n_frequencies, n_frames]   #(64,513,10)
    ...
    data_mono_batch = stft_mono_batch.transpose((0, 2, 1))  #(64,10,513)
